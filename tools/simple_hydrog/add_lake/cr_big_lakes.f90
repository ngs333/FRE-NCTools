...
integer :: i, j, n, l, k, m, ii, jj, j1, j2, i1, i2, id, jd, id_fine, jd_fine
integer :: idp1_fine, jdp1_fine, ktr2, ntiles, rcode, rcode2, varid, ncid
integer :: attnum, latid, lonid, ndm, londim, latdim, fill_val, latgid, longid
integer :: varid2, varid3, varid4, n1
real :: pi, dtr, sum, mval_tocell, mval_landf, mval_lmask, scale, sum1, asum1, f1  
character(len=8)   :: var
character(len=40)  :: var_units, lake_mask_var
character(len=200) :: fname, lmask_latlon_field, glcc_file, lname_glcc
...
real, allocatable, dimension (:,:)    :: interp_out, interp_mask, data_in
real, allocatable, dimension (:,:)    :: arlatf, lmask
real, allocatable, dimension (:,:,:)  :: lat, lon, tocell, land_frac, wbd, &
                                         lake_frac, lake_idx
character(len=100), allocatable, dimension (:) :: river_input_file

pi= 4.*atan(1.)
dtr= pi/180.
interp_glcc_to_1deg= .false.

open (10, file= 'out.cr_big_lakes', form= 'formatted')

read (5,*) ntiles
allocate (river_input_file(ntiles))
do n= 1,ntiles
...
allocate (tocell(id,jd,ntiles), land_frac(id,jd,ntiles))
...
allocate (wbd(id,jd,ntiles), interp_out(id,jd))
...
if (interp_glcc_to_1deg) then
    ...
    do n= 1,ntiles
       ...
       call horiz_interp (data_in, lonob*dtr, latob*dtr, lon(:,:,n)*dtr, &
          lat(:,:,n)*dtr, wbd(:,:,n), verbose=1, mask_in=interp_mask, &
          mask_out=interp_out, interp_method="bilinear")
       where (interp_out(:,:) == 0.) wbd(:,:,n)= mval_mdl
    enddo
else
    ...
    do n= 1,ntiles
       write (6,*) 'WaterBod, tile= ', n
       data_in= wbdat(:,:)
       call horiz_interp (data_in, lonlb*dtr, latlb*dtr, lon(:,:,n)*dtr, &
          lat(:,:,n)*dtr, wbd(:,:,n), verbose=1, mask_in=interp_mask, &
          mask_out=interp_out, interp_method="bilinear")
       where (interp_out(:,:) == 0.) wbd(:,:,n)= mval_mdl
    enddo
...
! now if lmask lat/lon field has value > 0, assign as lake in c720 field/tile
ktr2= 0
do j= 1,jd_fine
   do i= 1,id_fine
      if (lmask(i,j) > 0.3) then
             ...
             do jj= 1,jd
                do ii= 1,id
                   if (lat(ii,jj,n) > latfb(j) .and. lat(ii,jj,n) <= latfb(j+1) .and. &
                       ...
                       wbd(ii,jj,n) > 0.1 .and. land_frac(ii,jj,n) == 1.) then
                           ktr2= ktr2 + 1
                           il1(ktr2)= ii
                           jl1(ktr2)= jj
                           nl1(ktr2)= n
...
allocate (lake_idx(id,jd,ntiles))
ktr= 0 ;  ilake= 0 ;  jlake= 0 ;  nlake= 0
lake_frac= 0. ;  lake_idx= 0.
do l= 1,ktr2
   do n= 1,nlake_defp1
      if (idx1(l) == n) then
          ktr(n)= ktr(n) + 1
...
! first sort lake grid cells by tile number
do l= 1,nlake_defp1
   do n= 1,ktr(l)
      do m= 1,ktr(l)-1
         if (nlake(m,l) > nlake(m+1,l)) then
             n1= nlake(m,l)
             nlake(m,l)= nlake(m+1,l)
             nlake(m+1,l)= n1
...
! then sort lake grid cells by j
do l= 1,nlake_defp1
   do n= 1,ktr(l)
      do m= 1,ktr(l)-1
         if (nlake(m,l) == nlake(m+1,l).and. jlake(m,l) > jlake(m+1,l)) then
             j1= jlake(m,l)
             jlake(m,l)= jlake(m+1,l)
             jlake(m+1,l)= j1
...
do l= 1,nlake_defp1
   do n= 1,ktr(l)-1
      do m= 1,ktr(l)-1
         if (nlake(m,l) == nlake(m+1,l) .and. jlake(m,l) == jlake(m+1,l) .and. &
             ilake(m,l) > ilake(m+1,l)) then
             i1= ilake(m,l)
             ilake(m,l)= ilake(m+1,l)
             ilake(m+1,l)= i1
...
!    create data variable and attributes
   ...
   rcode= NF_PUT_ATT_TEXT (ncid, varid, 'long_name', 6, 'tocell')
   rcode= NF_PUT_ATT_TEXT (ncid, varid, 'units', 4, 'none')
   ...
   rcode= NF_DEF_VAR (ncid, 'lake_frac', NF_DOUBLE, 2, ndims, varid3)
   rcode= NF_PUT_ATT_TEXT (ncid, varid3, 'long_name', 9, 'lake_frac')
   rcode= NF_PUT_ATT_TEXT (ncid, varid3, 'units', 4, 'none')
   rcode= NF_PUT_ATT_DOUBLE (ncid, varid3, 'missing_value', NF_DOUBLE, 1, mval_mdl)
   
   rcode= NF_DEF_VAR (ncid, 'waterbod', NF_DOUBLE, 2, ndims, varid4)
   rcode= NF_PUT_ATT_TEXT (ncid, varid4, 'long_name', 8, 'waterbod')
...
!    tocell data
   start= 1 ;  count(1)= id ;  count(2)= jd 
   rcode= NF_PUT_VARA_DOUBLE (ncid, varid, start, count, tocell(:,:,n))

!    big lakes
   start= 1 ;  count(1)= id ;  count(2)= jd 
   rcode= NF_PUT_VARA_DOUBLE (ncid, varid3, start, count, lake_frac(:,:,n))

!    big lakes -- index
    start= 1 ;  count(1)= id ;  count(2)= jd 
    rcode= NF_PUT_VARA_DOUBLE (ncid, varid2, start, count, lake_idx(:,:,n))

!    waterbod
    start= 1 ;  count(1)= id ;  count(2)= jd 
    rcode= NF_PUT_VARA_DOUBLE (ncid, varid4, start, count, wbd(:,:,n))

!  close netcdf file
   rcode= NF_CLOSE (ncid)
enddo

deallocate (wbd, lake_idx)
deallocate (lat_idx, lon_idx)
deallocate (lat, lon, tocell, land_frac, lake_frac)
