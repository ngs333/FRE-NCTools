...
integer, parameter :: nlkmx= 150000
...
open (10, file= 'out.cr_lake_files_gage', form= 'formatted')
...
read (5,'(l1)') read_gage_data
read (5,'(l1)') cap_lake_depth
read (5,'(a)') glcc_file
close (5)
...
do n= 1,ntiles
   do j= 2,jdp1
      do i= 2,idp1
         if ( lake_frac(i,j,n) > 0 .and. &
             (wbd(i,j,n) == 0. .or. wbd(i,j,n) == mval_mdl)) then
              write (6,'(a,f9.4,a,f9.4,3i6)') 'WARNING (2): lake_frac= ', &
                lake_frac(i,j,n), ', waterbod= ', wbd(i,j,n), n, j-1, i-1
         endif
      enddo
   enddo
...
do n= 1,ntiles
   do j= 2,jdp1
      do i= 2,idp1
         if (lake_frac(i,j,n) > 0.) then
             if (wbd(i,j,n) < 0.03) write (6,'(a,3i5,2f9.4)') &
                "***WARNING***  Lake cell has small lake fraction, ", &
