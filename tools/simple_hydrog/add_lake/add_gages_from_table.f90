...
integer :: jgid, bgid, chdim, ngage
real :: pi, dtr, sum, mval_in
character(len=4)   :: b1
character(len=8)   :: var
...
integer, dimension (ngagemax)          :: igage, jgage

real, dimension (ngagemax)             :: lat_rg, lon_rg

character(len=4), dimension (ngagemax) :: bshort
...
rcode= NF_DEF_DIM (ncid, 'ngage',  ngage, ngdim)
rcode= NF_DEF_DIM (ncid, 'nchar_g',  len_bshort, chdim)
...
rcode= NF_PUT_VARA_DOUBLE (ncid, igid, start, count, real(igage))

start= 1 ;  count= 1 ;  count(1)= ngage
rcode= NF_PUT_VARA_DOUBLE (ncid, jgid, start, count, real(jgage))

do l= 1,ngage
   start(1)= 1 ;  count(1)= len_bshort
   start(2)= l ;  count(2)= 1
   rcode= NF_PUT_VARA_TEXT (ncid, bgid, start, count, bshort(l))
enddo
