...
integer :: varid2, varid3, varid4
real :: pi, dtr, sum, mval_frac, casp_ratio, mval_tocell, mval_landf
real :: mval_cella, mval_laka, mval_basin, ln1, lf, sum_casp
character(len=8)   :: var
character(len=40)  :: var_units
character(len=100) :: long_name, gridspec_file, fname
logical :: casp_input_from_model, read_lake_frac, dvlp
...
real, allocatable, dimension (:,:,:)   :: lat, lon, tocell, land_frac, cell_area, basin, &
                                          lake_area_g, area_g, lake_frac, lake_type, &
                                          lake_depth, lake_tau
...
read (5,'(l1)') casp_input_from_model
read (5,'(l1)') dvlp
if ( casp_input_from_model ) then
    do n= 1,ntiles
       read (5,'(a)') land_month_file(n)
    enddo
endif
...
write (6,*) 'casp_input_from_model= ', casp_input_from_model
write (6,*) 'development= ', dvlp
...
200 ktr= ktr + 1
    ...
    if ( casp_input_from_model ) then
        if (lake_area_g(i,j,n) /= mval_mdl) then
            ...
            sum_casp= sum_casp + lake_area_g(i,j,n)
        endif
...
where (lake_frac == 0.)
   ...
   lake_tau=    0.
endwhere
! comment this out for 'urgent' work, 13 nov 08
!where (lake_frac == 0.)
!   lake_type= 1.
!   lake_depth= lake_depth_phan
!   lake_tau= lake_tau_phan
!endwhere

if (dvlp) then
    where (lake_frac == 0.) lake_frac= 1.e-4
endif
...
!   if ( dvlp ) then
!      rcode= NF_DEF_VAR (ncid, 'lake_type', NF_DOUBLE, 2, ndims, varid2)
!      rcode= NF_PUT_ATT_TEXT (ncid, varid2, 'long_name', 9, 'lake_type')
...
!    lake type data
!      start= 1 ;  count(1)= id ;  count(2)= jd 
!      rcode= NF_PUT_VARA_DOUBLE (ncid, varid2, start, count, lake_type(:,:,n))
